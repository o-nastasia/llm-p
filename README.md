...
Проект представляет собой асинхронное FastAPI приложение, которое позволяет пользователям общаться с LLM через OpenRouter. Поддерживает регистрацию, JWT-авторизацию, сохранение истории диалогов в SQLite и управление креативностью ответов (temperature).
...
Создайте файл `.env`по примеру .env.example и укажите в нем свой реальный токен для `OPENROUTER_API_KEY`.
